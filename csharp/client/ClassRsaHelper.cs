...
namespace Crypto
{
    ...
    class AsnKeyBuilder
    {
        ...
        internal static AsnType CreateSequence(AsnType value)
        {
            ...
            System.Diagnostics.Debug.Assert(!IsEmpty(value));
        ...
        {
            // Should be at least 1...
            System.Diagnostics.Debug.Assert(!IsEmpty(values));
    ...
    class RSAHelper
    {
        ...
        static public RSAParameters ImportCryptoppPriKey(byte[] x509key)
        {
            var RSAparams = new RSAParameters();

            using (var binr = new BinaryReader(new MemoryStream(x509key)))
            {
                binr.ReadBytes(26); //discard first 26 bytes, they are NO use
                byte bt = 0;
                ushort twobytes = 0;
        ...
        static public RSAParameters ImportCryptoppPubKey(byte[] x509key)
        {
            // encoded OID sequence for  PKCS #1 rsaEncryption szOID_RSA_RSA = "1.2.840.113549.1.1.1"
            byte[] SeqOID = { 0x30, 0x0D, 0x06, 0x09, 0x2A, 0x86, 0x48, 0x86, 0xF7, 0x0D, 0x01, 0x01, 0x01, 0x05, 0x00 };
            byte[] seq = new byte[15];
            int x509size;
            RSAParameters nullpara = new RSAParameters();
        ...
        static public byte[] Encrypt(byte[] DataToEncrypt, RSAParameters RSAKeyInfo)
        {
            try
            {
                ...
                using (RSACryptoServiceProvider RSA = new RSACryptoServiceProvider())
                {
                    ...
                    using (MemoryStream PlaiStream = new MemoryStream(DataToEncrypt))
        ...
        static public byte[] Decrypt(byte[] DataToDecrypt, RSAParameters RSAKeyInfo)
        {
            try
            {
                ...
                using (RSACryptoServiceProvider RSA = new RSACryptoServiceProvider())
                {
                    ...
                    if (DataToDecrypt.Length <= MaxBlockSize)
                    {
                        return RSA.Decrypt(DataToDecrypt, RSAEncryptionPadding.Pkcs1);
                    }

                    using (MemoryStream CrypStream = new MemoryStream(DataToDecrypt))
        ...
        static public byte[] Encrypt(byte[] DataToEncrypt, byte[] RSAKeyInfo)
        {
            try
            {
                //Create a new instance of RSACryptoServiceProvider.
        ...
        static public byte[] Decrypt(byte[] DataToDecrypt, byte[] RSAKeyInfo)
        {
            try
            {
                //Create a new instance of RSACryptoServiceProvider.
